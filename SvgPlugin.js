/*!
 * GSAP SVG Plug-in
 * Version 0 (i.e., totally experimental and probably broken somehow)
 *
 * A mod of GSAP's RaphaelPlugin--relevant licencing info:
...
(window._gsQueue || (window._gsQueue = [])).push( function() {
	...
	var	_NaNExp = /[^\d\-\.]/g,
		...
		SvgPlugin = window._gsDefine.plugin({
			propName: "svg",
			API: 2,
			...
			init: function(target, value, tween) {
				...
				for (p in value) {
					...
					this._overwriteProps.push("svg_" + p);
					if (pt._next) {
						pt._next._prev = pt;
					}
				}
			...
			set: function(v) {
				...
				if (this._transform) {
					...
					this._target.attr("transform","matrix(" + a + "," + b + "," + c + "," + d + "," + (pt.tx + pt.ox) + "," + (pt.ty + pt.oy) + ")");
				}

			}

		}),
		p = SvgPlugin.prototype;
	...
	p._parseTransform = function(t, v) {
		...
		if (typeof(v) === "object") { //for values like scaleX, scaleY, rotation, x, y, skewX, and skewY or transform:{...} (object)
			...
			if (typeof(pivot) === "string") {
				...
				// THIS WILL BREAK!
				// HAVE NOT YET FOUND A NATIVE ALTERNATIVE TO RAPHAEL'S getBBox(true)
				copy = t.getBBox(true);
				dx = copy.width / 2;
				dy = copy.height / 2;
			} else {
